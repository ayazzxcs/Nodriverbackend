...
class LocalAuthProxyBridge:
    """
    Local unauthenticated HTTP proxy for Chrome.
    It forwards traffic to authenticated upstream proxy:
    http://username:password@host:port
    """
    def __init__(self, upstream_proxy):
        ...
        if not parsed.hostname or not parsed.port:
            raise ValueError(f"Invalid upstream proxy: {upstream_proxy}")
    ...
    async def start(self):
        ...
        print(f"Starting built-in proxy bridge on {local}")
    ...
    async def handle_client(self, client_reader, client_writer):
        ...
        try:
            ...
            header_text = first.decode("iso-8859-1", errors="ignore")
            ...
            if self.auth_header and not has_auth:
                new_lines.insert(1, self.auth_header.strip())
...
async def start_proxy_bridge(upstream_proxy):
    if not upstream_proxy:
        return None, None

    bridge = LocalAuthProxyBridge(upstream_proxy)
    local_proxy = await bridge.start()
    return bridge, local_proxy


async def stop_proxy_bridge(bridge):
    if bridge:
        await bridge.stop()
...
async def start_browser(local_proxy=None):
    args = [
        "--no-sandbox",
    ...
    print(f"Chrome proxy: {'local bridge configured' if local_proxy else 'none'}")
    print("Starting Nodriver with headless=True, no_sandbox=True")
...
async def verify_proxy(browser):
    try:
        ...
        print(f"Proxy verification IP/page text: {ip_text[:200]}")
    except Exception as e:
        print(f"Proxy verification failed: {e}")


async def verify_browser(browser):
    try:
        page = await browser.get("https://example.com")
        await sleep_ms(2000)
        text = (await get_text(page)).strip()
        print(f"Browser verification text: {text[:200]}")
    except Exception as e:
...
async def main():
    ...
    try:
        for i in range(START_INDEX, end_index):
            ...
            upstream_proxy = proxy_for_index(i, proxies)

            if browser is None or upstream_proxy != active_proxy:
                if browser:
                    try:
                        await browser.stop()
                    except Exception:
                ...
                await stop_proxy_bridge(proxy_proc)
                proxy_proc = None

                active_proxy = upstream_proxy
                print(f"Starting Nodriver browser with proxy: {'configured' if upstream_proxy else 'none'}")

                proxy_proc, local_proxy = await start_proxy_bridge(upstream_proxy)
                browser = await start_browser(local_proxy)
                await verify_browser(browser)
    ...
    write_json(META_PATH, {
        ...
        "mode": "nodriver-only google lens + amazon scraping + built-in auth proxy bridge",
        "startIndex": START_INDEX,
        "maxProducts": MAX_PRODUCTS,
        "matches": len(matches),
        "failures": len(failures),
        "proxyCount": len(proxies),
        "note": "Uses built-in Python local bridge for authenticated residential proxies. Chrome only sees localhost proxy."
    })
