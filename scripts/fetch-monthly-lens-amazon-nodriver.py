...
def start_proxy_bridge(upstream_proxy):
    """
    Chrome does not reliably accept http://user:pass@host:port in --proxy-server.
    This starts a local unauthenticated proxy and forwards it to the authenticated Webshare proxy.
    ...
    print(f"Starting local proxy bridge on {local}")
...
async def main():
    ...
    write_json(META_PATH, {
        ...
        "mode": "nodriver-only google lens + amazon scraping + local auth proxy bridge",
        "startIndex": START_INDEX,
        "maxProducts": MAX_PRODUCTS,
        "matches": len(matches),
        "failures": len(failures),
        "proxyCount": len(proxies),
        "note": "Uses pproxy local bridge for authenticated residential proxies. Chrome only sees localhost proxy."
    })
